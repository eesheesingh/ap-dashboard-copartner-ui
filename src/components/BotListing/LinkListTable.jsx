        ...
        <table className="min-w-full divide-y divide-gray-700">
          ...
          <tbody className="bg-transparent divide-y divide-gray-700">
            {tableData.map((user, index) => (
              ...
              >
                <td className="px-6 py-4 whitespace-nowrap md:text-[18px] text-center relative">
                  <input
                    type="checkbox"
                    checked={isSelected(index)}
                    onChange={() => handleSelectRow(index)}
                  />
                  {isSelected(index) && <FaCheckCircle className="absolute top-1/2 left-4 transform -translate-y-1/2 text-[#fff]" />}
                </td>
                <td className="px-6 py-4 whitespace-nowrap md:text-[18px] text-center">{user.date}</td>
                <td className="px-6 py-4 whitespace-nowrap md:text-[18px] text-center">{user.mobile}</td>
                <td className="px-6 py-4 whitespace-nowrap md:text-[18px] text-center">{user.verified}</td>
                <td className="px-6 py-4 whitespace-nowrap md:text-[18px] text-center">{user.joined}</td>
              </tr>
            ))}
          </tbody>
        </table>
